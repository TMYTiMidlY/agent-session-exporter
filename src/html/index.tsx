...
const PILL_DEFS: Record<FilterKey, PillDef> = {
  ...
  assistant:     { key: "assistant",     label: "助手",     icon: "bot",           accent: "green" },
...
function Report({ session, summary, sourceLabel }: { session: ParsedSession; summary?: string; sourceLabel?: string }) {
  ...
  const sourceWarning = lossySourceWarning(session.source)
    ?? sourceLabelWarning(sourceLabel);
      ...
      <body>
        <header className="topbar">
          <div className="title-row">
            ...
            <div className="meta">
              ...
              {sourceWarning && (
                <span className="fallback-warning" title="该数据源存在已知的保真限制">
                  ⚠ {sourceWarning}
                </span>
              )}
            </div>
              ...
              {visiblePills.map((key) => {
                ...
                return (
                  <button key={key} type="button" className={`filter accent-${def.accent}`} data-filter={key} aria-pressed="true">
                    <Icon name={def.icon} />
                    <span>{pillLabel(key, session.agent)}</span>
        ...
        <div className="layout">
              ...
              {session.entries.map((entry) => {
                ...
                return (
                  <a key={entry.index} href={`#entry-${entry.index}`} data-nav-role={key}>
                    ...
                    <strong>{pillLabel(key, session.agent)}</strong>
                    <em>{text}</em>
                  </a>
                );
              })}
          ...
          <main>
            ...
            {session.entries.map((entry) => (
              <Entry key={entry.index} entry={entry} sessionStart={sessionStart} agent={session.agent} />
            ))}
            <footer>
...
function Entry({ entry, sessionStart, agent }: { entry: TimelineEntry; sessionStart?: string; agent: AgentKind }) {
  ...
  if (entry.role === "assistant") {
    return <SimpleEntry entry={entry} isMarkdown icon="bot" sessionStart={sessionStart} label={assistantLabel(agent)} />;
  }
...
function SimpleEntry({ entry, isMarkdown, icon: iconName, sessionStart, label }: {
  entry: TimelineEntry;
  isMarkdown: boolean;
  icon: string;
  sessionStart?: string;
  ...
  return (
    <details {...entryAttrs(entry, key)} open={defaultOpen(entry)}>
      <summary>
        ...
        <Icon name={iconName} />
        <span className="role">{label ?? PILL_DEFS[key].label}</span>
...
function resultIcon(kind: ToolResultKind): string {
  ...
  if (kind === "redacted") return "circle-dashed";
  return "hourglass";
}
...
function pillLabel(key: FilterKey, agent: AgentKind): string {
  return key === "assistant" ? assistantLabel(agent) : PILL_DEFS[key].label;
}
...
.entry.tool.tool-redacted { border-left-color:var(--gray); }
...
.tool-status.status-redacted { color:var(--gray); }
